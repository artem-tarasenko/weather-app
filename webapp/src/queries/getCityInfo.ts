...
export async function fetchCitiesSuggestions(city: string): Promise<Array<ICityResponseInfo> | null> {
...
export function useCitiesSearchQuery(city: string) {
    return useQuery<Array<ICityResponseInfo> | null>({
        queryKey: ['cities', city],
        queryFn: () => fetchCitiesSuggestions(city),
    });
}
